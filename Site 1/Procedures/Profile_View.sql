...
begin
	...
	union
	(select display_name, user_name, score, player_level, bio 
	from Player_Info@site_link where player_id = player_id_input)
	);
	
	select count(match_id) into total_match from Challenge_Game_Info
	where player1 = player_id_input or player2 = player_id_input;
	
	select count(match_id) into won from Challenge_Game_Info
	where winner = player_id_input;
	
	select count(match_id) into lost from Challenge_Game_Info
	where ((player1 = player_id_input or player2 = player_id_input)
	and winner != player_id_input and winner != -100);
	
	select count(match_id) into draw from Challenge_Game_Info
	where ((player1 = player_id_input or player2 = player_id_input)
	and winner = -100);
